...
def send_apprise_notification(url: str, title: str, body: str) -> tuple[bool, str]:
    ...
    if target.startswith("qmsg://"):
        try:
            ...
            form_data: dict[str, str] = {"msg": msg or "Research Claw Notification"}
    ...
    if target.startswith("feishu-webhook://"):
        try:
            ...
            if body_text:
                msg = f"{msg}\n\n{body_text}" if msg else body_text
            payload = json.dumps(
                {"msg_type": "text", "content": {"text": msg or "Research Claw Notification"}},
                ensure_ascii=False,
            ).encode("utf-8")
    ...
    if target.startswith("dingtalk-webhook://"):
        try:
            ...
            if body_text:
                msg = f"{msg}\n\n{body_text}" if msg else body_text
            payload = json.dumps(
                {"msgtype": "text", "text": {"content": msg or "Research Claw Notification"}},
                ensure_ascii=False,
            ).encode("utf-8")
            req = Request(actual_url, data=payload, method="POST")
            req.add_header("Content-Type", "application/json")
    ...
    ok = app.notify(title=title or "Research Claw Notification", body=body or "")
...
def _send_email_sync(recipient: str, title: str, body: str, smtp_profile: Optional[dict[str, Any]] = None) -> None:
    ...
    msg = EmailMessage()
    msg["Subject"] = title or "Research Claw Notification"
    msg["From"] = f"{from_name} <{sender}>" if from_name and sender else sender
    msg["To"] = to_addr
    msg.set_content(body or title or "Research Claw Notification")
