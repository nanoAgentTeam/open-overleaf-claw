...
推荐优先使用统一配置字段 `channel.accounts` 与 `channel.activeId` 管理 IM 账号；旧版 `channels.feishu / channels.telegram` 仅在 `channel.accounts` 为空时作为兼容回退路径。
...
0 9 * * * cd /path/to/context_bot_project && python cli/main.py agent -m "调研 MoE 领域最新论文" -p MyPaper
```
