...
2. **Project Memory**: Stores session-specific tools calls and deep investigation notes."""
        ...
        # Inject only compact index into system prompt, then resolve full content via memory tools by ID.
        if not is_default_project and self._project:
            try:
                resolved_session_id = str(getattr(self._session, "id", "") or session_id).strip().lower()
                if resolved_session_id == "automation":
                    ...
                    store = ProjectKnowledgeStore(self._project)
